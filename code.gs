...
const formulaMapLPG = {
  ...
  for (let i = CONFIG.dataStartRow - 1; i < stopRow; i++) {
    ...
    const r = i + 1;
    const normalizedLabel = rowLabel.replace(/[^a-z]/g, ""); // "sub-total" -> "subtotal"

    // 1. Identify Subtotals
    if (normalizedLabel.includes("subtotal")) {
      ...
      sectionStartRow = r + 1; // The next section starts after this subtotal row
    }

    // 2. Identify Final Total
    if (normalizedLabel === "total") {
      sumCols.forEach(col => {
        let formula = "";
        if (subTotalRowsFound.length > 0) {
          // If subtotals exist, sum only those subtotal rows (Prevents double counting)
          let refs = subTotalRowsFound.map(subR => `${col}${subR}`).join(",");
          formula = `=SUM(${refs})`;
        } else {
          // If NO subtotals found, sum the entire column from top to bottom
          const rangeEnd = r - 1;
...
function scanTab(tabName, shouldClearLogs = true) {
  ...
  for (let i = 0; i < dataValues.length; i++) {
    ...
    const labelA = String(row[0]).toLowerCase().trim();
      ...
      if (normalizedLabelA.includes("Total") || labelA.trim() === "") {
        continue;
      }
