...
function recordActivePBTT() {
  ...
  if (!p || !l || !s || !e) { 
    SpreadsheetApp.getUi().alert("Missing Header Info (E4, E5, E7, E8)"); 
    return; 
  }
  try {
    ...
    // Format timestamp and E7/E8 as "MMM d, yyyy"
    const formattedTimestamp = Utilities.formatDate(new Date(),"Asia/Manila","MMM d, yyyy hh:mm a");
    const formattedS = Utilities.formatDate(new Date(s), Session.getScriptTimeZone(), "MMM d, yyyy");
    const formattedE = Utilities.formatDate(new Date(e), Session.getScriptTimeZone(), "MMM d, yyyy");

    dSh.appendRow([formattedTimestamp, p, l, formattedS, formattedE, ss.getUrl(), Session.getActiveUser().getEmail()]);
    SpreadsheetApp.getUi().alert("Recorded!");
  } catch(x) { 
    SpreadsheetApp.getUi().alert("Err: "+x.message); 
  }
}
